            ...
            parts.forEach(part => {
                if (part.text.match(/[()\[\]{}]/)) {
                    for (let char of part.text) {
                        ...
                        if (part.isToken) span.style.color = part.color || 'inherit';
                    ...
                    if (part.isToken) node.style.color = part.color || 'inherit';
...
const applyHighlight = (editor: any, textarea: HTMLTextAreaElement, lineElements: any[]) => {
    ...
    requestAnimationFrame(() => {
        ...
        containers.querySelectorAll('.bracket-match').forEach((el: HTMLElement) => el.classList.remove('bracket-match'));
